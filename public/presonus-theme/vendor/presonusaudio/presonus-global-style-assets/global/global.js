var presonus = {
    ...
    buildLanguageSelectorMessage: function( language )
    {
	    ...
	    if ( language == 'german' ) {
		    ...
		    html += '<p>Einige oder alle der Inhalt dieser Seite ist in mehreren Sprachen verfÃ¼gbar. WÃ¤hlen Sie unter Sprache. Wenn der Inhalt nicht verfÃ¼gbar ist, wird es in englischer Sprache angezeigt.</p>';
	    } else if ( language == 'spanish' ) {
		    html = '<p>Su lengua se establece en <strong>EspaÃ±ol</strong></p>';
		    html += '<p>Algunos o todos el contenido de esta pÃ¡gina estÃ¡ disponible en varios idiomas. Seleccione su idioma a continuaciÃ³n. Si el contenido no estÃ¡ disponible, se mostrarÃ¡ en InglÃ©s.</p>';
	    } else if ( language == 'japanese' ) {
		    html = '<p>ãŠä½¿ã„ã®è¨€èªžãŒ<strong>æ—¥æœ¬èªž</strong>ã«è¨­å®šã•ã‚Œã¦ã„ã¾ã™</p>';
		    html += '<p>ã“ã®ãƒšãƒ¼ã‚¸ã®ã‚³ãƒ³ãƒ†ãƒ³ãƒ„ã®ä¸€éƒ¨ã¾ãŸã¯å…¨éƒ¨ã‚’è¤‡æ•°ã®è¨€èªžã§åˆ©ç”¨å¯èƒ½ã§ã™ã€‚ä¸‹è¨˜ã®è¨€èªžã‚’é¸æŠžã—ã¦ãã ã•ã„ã€‚ã‚³ãƒ³ãƒ†ãƒ³ãƒ„ãŒä½¿ç”¨ã§ããªã„å ´åˆã€ãã‚Œã¯ã€è‹±èªžã§è¡¨ç¤ºã•ã‚Œã¾ã™ã€‚</p>';
	    } else if ( language == 'chinese' ) {
		    html = '<p>æ‚¨çš„è¯­è¨€è®¾ç½®ä¸º<strong>ä¸­å›½</strong></p>';
		    html += '<p>éƒ¨åˆ†æˆ–å…¨éƒ¨æ­¤é¡µé¢ä¸Šçš„å†…å®¹æ˜¯æä¾›å¤šè¯­è¨€ç‰ˆæœ¬ã€‚åœ¨ä¸‹é¢é€‰æ‹©æ‚¨çš„è¯­è¨€ã€‚å¦‚æžœå†…å®¹ä¸å¯ç”¨ï¼Œå®ƒå°†ä»¥è‹±æ–‡æ˜¾ç¤ºã€‚</p>';
	    } else if ( language == 'french' ) {
		    html = '<p>Votre langue est le <strong>franÃ§ais</strong></p>';
		    html += '<p>Certains ou la totalitÃ© du contenu de cette page est disponible en plusieurs langues. Choisissez votre langue ci-dessous. Si le contenu ne sont pas disponibles, il sera affichÃ© en anglais.</p>';
	    } else if ( language == 'turkish' ) {
		    html = '<p>Diliniz TÃ¼rkÃ§e olarak ayarlandÄ±.</p>';
		    html += '<p>Certains ou la totalitÃ© du contenu de cette page est disponible en plusieurs langues. Choisissez votre langue ci-dessous. Si le contenu ne sont pas disponibles, il sera affichÃ© en anglais.</p>';
	    }
	    if ( html == '' ) {
